...
function sortFiles(
  ...
  sorted.sort((a, b) => {
    ...
    if (sort.column === "modified") {
      const diff = new Date(a.mtime).getTime() - new Date(b.mtime).getTime();
...
export const FolderTableView: React.FC<FolderTableViewProps> = ({
  folderPath,
  onSelectFile,
}) => {
  const [data, setData] = useState<FolderTableData | null>(null);
  ...
  useEffect(() => {
    ...
    window.api
      ...
      .catch((err) => {
        if (!stale) {
          console.error("Failed to load folder table:", err);
  ...
  useEffect(() => {
    return window.api.onFsChanged((events) => {
      const relevant = events.some((e) => e.dirPath === folderPath);
      if (relevant) {
        window.api
          .readFolderTable(folderPath)
          .then((result) => {
  ...
  const folderName = useMemo(() => extractFolderName(folderPath), [folderPath]);
  ...
  if (error) {
    return (
      <div className="folder-table-view">
        <div className="folder-table-heading">{folderName}</div>
        <div className="folder-table-empty">{error}</div>
      </div>
    );
  }
  ...
  if (data.files.length === 0) {
    return (
      <div className="folder-table-view">
        <div className="folder-table-heading">{folderName}</div>
        <div className="folder-table-empty">No files in this folder</div>
      </div>
    );
  }
    ...
    <div className="folder-table-view">
      ...
      <div className="folder-table-scroll scrollbar-hover">
        <table className="folder-table">
          <thead>
            <tr>
              <th onClick={() => handleHeaderClick("filename")}>
                Name
                {renderSortIndicator("filename")}
              </th>
              <th onClick={() => handleHeaderClick("modified")}>
                Modified
          ...
          <tbody>
            {sortedFiles.map((file) => (
              <tr key={file.path} onClick={() => onSelectFile(file.path)}>
                <td title={file.filename}>{file.filename}</td>
                <td>{formatDate(file.mtime)}</td>
                {data.columns.map((col) => (
                  <td key={col}>{formatCellValue(file.frontmatter[col])}</td>
                ))}
              </tr>
            ))}
          </tbody>
