...
interface FolderRowProps {
  ...
  onAliasConfirm?: () => void;
  onAliasCancel?: () => void;
  onContextMenu?: (e: React.MouseEvent, item: FlatItem | null) => void;
...
const WorkspaceLabel = React.memo(function WorkspaceLabel({
  ...
  return (
    <div className="workspace-label">
      ...
      <span className="workspace-name workspace-name-real">{item.name}</span>
    </div>
  );
});
...
export const FolderRow = React.memo(function FolderRow({
  ...
  return (
    <div
      ...
      onDragStart={isWorkspace ? undefined : (e) => onDragStart?.(e, item.path)}
      onDragOver={(e) => onDragOver?.(e, item.path)}
      onDragLeave={onDragLeave}
      onDrop={(e) => onDrop?.(e, item.path)}
      onDragEnd={onDragEnd}
      onClick={(e) => onToggle(item.path, e.altKey)}
      onContextMenu={(e) => {
        e.preventDefault();
    ...
    >
      <span
        className="collection-tree-caret"
        style={hideChevron ? { visibility: "hidden" } : undefined}
      >
        {item.isExpanded ? (
      ...
      <button
        ...
        onClick={(e) => {
          e.stopPropagation();
          window.api.openInTerminal(item.path);
        }}
      >
...
export const FileRow = React.memo(
  function FileRow({
    ...
    const fileName = isSearchResult ? item.name.slice(slash + 1) : item.name;
    const parentDir = isSearchResult ? item.name.slice(0, slash + 1) : "";
    ...
    return (
      <div
        ...
        onMouseLeave={() => {
          if (isDeleteConfirm) onDeleteCancel?.();
        }}
      >
        <span className="item-icon">
          {thumbnailUrl ? (
            ...
            (() => {
              ...
              return (
                <IconComp size={ICON_SIZE} weight="regular" style={{ color }} />
              );
            })()
          )}
        </span>
        {isAliasing ? (
          ...
          <div className="search-result-label">
            <span className="search-result-parent">{parentDir}</span>
...
interface TreeViewProps {
  ...
  onToggleFolder: (path: string, recursive: boolean) => void;
  onCreateFile: (folderPath: string, name: string) => void;
  onPlusClick?: (folderPath: string) => void;
  onContextMenu?: (e: React.MouseEvent, item: FlatItem | null) => void;
...
export const TreeView: React.FC<TreeViewProps> = ({
  ...
  const handleDelete = useCallback(
    (e: React.MouseEvent, filePath: string) => {
      e.preventDefault();
      e.stopPropagation();
      if (deleteConfirmRef.current === filePath) {
  ...
  const [folderRowHeight, setFolderRowHeight] = useState(0);

  useLayoutEffect(() => {
    if (folderRowHeight > 0 || !containerRef.current) return;
    const el = containerRef.current.querySelector(".collection-folder-row");
    if (el) {
      setFolderRowHeight(el.getBoundingClientRect().height);
    }
  ...
  const renderItems = (
    ...
    while (i < flatItems.length) {
      ...
      if (item.kind === "folder" && item.isExpanded) {
        i++;
        const [children, nextI] = renderItems(i, item.level + 1);
        const guideStyle = {
        ...
        nodes.push(
          <div key={item.id} className="folder-group" style={guideStyle}>
            <FolderRow
              item={item}
        ...
        nodes.push(
          <FileRow
            ...
            isMultiSelected={
              selectedPaths.has(item.path) && item.path !== selectedPath
            }
            isDeleteConfirm={deleteConfirmId === item.path}
