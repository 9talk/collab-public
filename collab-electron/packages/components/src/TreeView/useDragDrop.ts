...
export function useDragDrop(
  onMove: (sourcePaths: string[], targetFolder: string) => Promise<void>,
  ...
  const handleDragStart = useCallback(
    (e: React.DragEvent, path: string, selectedPaths: Set<string>) => {
      const paths = selectedPaths.has(path) ? [...selectedPaths] : [path];
      draggedPathsRef.current = paths;
      dragSourceWorkspaceRef.current = getWorkspacePath?.(path);
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent, targetFolder: string) => {
      ...
      const isInvalid = paths.some(
        (p) => p === targetFolder || targetFolder.startsWith(p + "/"),
      );
      if (isInvalid) return;
