...
export interface WorkspaceTreeProps {
  ...
  isExpanded: boolean;
  onToggleExpand: (path: string, recursive: boolean) => void;
...
function flattenAllFiles(nodes: TreeNode[], workspacePath: string): FlatItem[] {
...
>(function WorkspaceTree(
  ...
  const {
    ...
    collapseAllDirs,
    navigableItems,
  } = useWorkspaceFileTree(workspace.path, sortMode);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    window.api.readTree({ root: workspace.path }).then((tree: TreeNode[]) => {
      if (cancelled) return;
      setAllFiles(flattenAllFiles(tree, workspace.path));
    });
    return () => {
      cancelled = true;
    };
  ...
  useImperativeHandle(
    ...
    () => ({
      flatItems,
      navigableItems: isSearching || listView ? filteredItems : navigableItems,
      expandAncestors,
      expandRecursive,
      collapseAllDirs,
    }),
  ...
  const groupedFlatItems = useMemo(() => {
    ...
    const map = new Map<
      string,
      { key: string; label: string; items: FlatItem[] }
    >();
    ...
    if (sortMode.startsWith("alpha")) {
      ...
      for (const item of filteredItems) {
        ...
        const tsStr =
          typeof ts === "number" ? new Date(ts).toISOString() : (ts ?? "");
  ...
  return (
    <div className={`workspace-group${isExpanded ? "" : " collapsed"}`}>
      <FolderRow
        item={workspaceItem}
        onToggle={(path, recursive) => onToggleExpand(path, recursive)}
        ...
        isDropTarget={dropTargetPath === workspace.path}
      ...
      groupedFlatItems ? (
        groupedFlatItems.map((group) => (
          <div key={group.key}>
            <div className="list-date-separator">{group.label}</div>
            <TreeView
