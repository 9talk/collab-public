...
export function useWorkspaceFileTree(
  ...
  const [dirContents, setDirContents] = useState<Map<string, TreeNode[]>>(
    new Map(),
  );
  const [expandedDirs, setExpandedDirs] = useState<Set<string>>(new Set());
  const dirContentsRef = useRef(dirContents);
  dirContentsRef.current = dirContents;
  const pendingLoadsRef = useRef(new Map<string, Promise<TreeNode[]>>());
  ...
  const loadDir = useCallback(async (dirPath: string) => {
      ...
      try {
        ...
        const children: TreeNode[] = entries
          ...
          .sort((a: TreeNode, b: TreeNode) => {
            const aIsDir = a.kind === "folder";
            const bIsDir = b.kind === "folder";
    ...
    pendingLoadsRef.current.set(dirPath, request);
    return request;
  }, []);
  ...
  useEffect(() => {
    window.api
      ...
      .then((enabled) => {
        console.log(
          "[useWorkspaceFileTree] rememberExpandedDirs =",
          enabled,
          "workspace =",
          workspacePath,
        );
        if (enabled === false) return [];
        return window.api.getWorkspacePref("expanded_dirs", workspacePath);
      })
      .then((dirs) => {
        if (Array.isArray(dirs)) {
          console.log(
            "[useWorkspaceFileTree] restoring expandedDirs count =",
            dirs.length,
          );
          setExpandedDirs(new Set(dirs as string[]));
        }
      })
      .catch(() => {});
  ...
  useEffect(() => {
    return window.api.onFsChanged((events) => {
      const affectedDirs = new Set(events.map((e) => e.dirPath));
      const toReload = new Set<string>();
      for (const dirPath of affectedDirs) {
        if (dirPath !== workspacePath && !isSubpath(workspacePath, dirPath))
          continue;
        if (
          ...
          while (true) {
            ...
            parent = nextParent;
            if (dirContentsRef.current.has(parent)) {
              toReload.add(parent);
              break;
            }
          }
  ...
  const hydratedTree = useMemo(() => {
    ...
    return children.map((child) => hydrateNode(child, dirContents));
  }, [workspacePath, dirContents]);

  const flatItems = useMemo(() => {
    const items = flattenTree(hydratedTree, expandedDirs, 0, sortMode, 1);
    return items.map((item) => ({
      ...item,
      workspacePath,
    }));
  }, [hydratedTree, expandedDirs, sortMode, workspacePath]);

  const navigableItems = useMemo(
    () => flatItems.filter((item) => item.kind === "file"),
    [flatItems],
  );
  ...
  const expandRecursive = useCallback(
    async (rootPath: string) => {
      ...
      async function collect(path: string) {
        ...
        const subs = children.filter((n) => n.kind === "folder");
        await Promise.all(subs.map((s) => collect(s.path)));
      }
  ...
  const collapseAllDirs = useCallback(() => {
    setExpandedDirs((prev) => {
      ...
      for (const p of prev) {
        if (p !== workspacePath && !isSubpath(workspacePath, p)) {
          next.add(p);
        }
      }
      saveExpandedDirs(next, workspacePath);
  ...
  const toggleExpand = useCallback(
    ...
    [dirContents, loadDir, expandedDirs, expandRecursive, workspacePath],
  );
  ...
  const expandAncestors = useCallback(
    (filePath: string) => {
      if (!isSubpath(workspacePath, filePath)) return;

      const relative = filePath.slice(workspacePath.length + 1);
      ...
      setExpandedDirs((prev) => {
        if (dirsToExpand.every((p) => prev.has(p))) return prev;
        const next = new Set(prev);
        for (const p of dirsToExpand) next.add(p);
        saveExpandedDirs(next, workspacePath);
        return next;
      });
