...
export function WorkspaceGraph({
  ...
  const mergedData = useMemo(() => {
    ...
    for (const [sessionId, session] of agentSessions) {
      ...
      const seenFiles = new Map<
        string,
        { touchType: string; timestamp: number }
      >();
  ...
  const scopedData = useMemo(() => {
    ...
    const scopedLinks = mergedData.links.filter((l) => {
  ...
  useEffect(() => {
    ...
    Promise.all(
      imageNodes.map(async (node) => {
        try {
          const url = await window.api.getImageThumbnail(node.path, 192);
  ...
  const nodeStrokeWidthFn = useCallback((node: GraphNode) => {
  ...
  const nodeRadius = useCallback(
    (node: GraphNode) => {
      ...
      const t = Math.max(
        ...
        Math.min(
          1,
          (w - weightRange.min) / (weightRange.max - weightRange.min),
        ),
      );
  ...
  const timelineElement =
    !replay.isGitRepo || !atGitRoot ? null : (
      <ReplayTimeline
