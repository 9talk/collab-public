...
export interface ForceGraphOptions {
  ...
  nodeTitle?: (node: GraphNode, index: number) => string | null | undefined;
  nodeRadius?: number | ((node: GraphNode, index: number) => number);
  nodeFill?: string | ((node: GraphNode, index: number) => string);
  nodeStroke?: string | ((node: GraphNode, index: number) => string);
  nodeStrokeWidth?: number | ((node: GraphNode, index: number) => number);
  ...
  linkStrokeWidth?: number | ((link: GraphLink, index: number) => number);
...
export interface ForceGraphHandle {
  ...
  updateNodeFill: (fn: (node: GraphNode, index: number) => string) => void;
  updateNodeStroke: (fn: (node: GraphNode, index: number) => string) => void;
  updateNodeStrokeWidth: (
    fn: (node: GraphNode, index: number) => number,
  ) => void;
  updateLinkStroke: (fn: (link: GraphLink, index: number) => string) => void;
  resetView: () => void;
...
export function createForceGraph(
  ...
  let nodeTitles = nodeTitle
    ? currentNodes.map((node, index) => nodeTitle(node, index))
    : null;
  let radiusValues =
    typeof nodeRadius === "function"
      ? currentNodes.map((node, index) => nodeRadius(node, index))
      : null;
  let nodeFillValues =
    typeof nodeFill === "function"
      ? currentNodes.map((node, index) => nodeFill(node, index))
      : null;
  let nodeStrokeValues =
    typeof nodeStroke === "function"
      ? currentNodes.map((node, index) => nodeStroke(node, index))
      : null;
  let nodeStrokeWidthValues =
    typeof nodeStrokeWidth === "function"
      ? currentNodes.map((node, index) => nodeStrokeWidth(node, index))
      : null;
  ...
  let linkStrokeColors =
    typeof linkStroke === "function" ? d3.map(currentLinks, linkStroke) : null;

  let forceNodes: ForceNode[] = currentNodes.map((node, index) => ({
    ...node,
    id: String(nodeIds[index]),
    __index: index,
  }));

  let forceLinks: ForceLink[] = currentLinks.map((link, index) => ({
    ...link,
    source: linkSources[index],
    target: linkTargets[index],
  }));

  const getNodeRadius = (_d: ForceNode, index: number) => {
  ...
  let currentTheme: ForceGraphThemeOptions = {
    nodeFill: typeof nodeFill === "string" ? nodeFill : undefined,
    nodeStroke,
    nodeStrokeOpacity,
    nodeStrokeWidth,
    linkStroke: typeof linkStroke === "string" ? linkStroke : undefined,
    linkStrokeOpacity,
  };
  ...
  const getNodeFill = (d: ForceNode, index: number) => {
    ...
    if (typeof nodeFill === "function") {
      return (
        ...
        (nodeFillValues ? nodeFillValues[index] : "currentColor")
      );
    }
    return nodeFillValues ? nodeFillValues[index] : "currentColor";
  };

  const getLinkStrokeWidth = (_d: ForceLink, index: number) => {
    if (typeof linkStrokeWidth === "number") {
      return linkStrokeWidth;
    }
    return linkStrokeWidths ? linkStrokeWidths[index] : 1.5;
  };

  const getLinkStrokeColor = (_d: ForceLink, index: number) => {
    if (typeof linkStroke === "string") {
      return linkStroke;
    }
    return linkStrokeColors ? linkStrokeColors[index] : "#999";
  };
  ...
  if (
    ...
    simulation.force("centerX", piecewiseX).force("centerY", piecewiseY);
  } else {
  ...
  tooltip.setAttribute("data-forcegraph-tooltip", "true");
  ...
  const positionTooltip = (event: PointerEvent) => {
    ...
    const x = event.clientX - rootRect.left + offsetX;
    const y = event.clientY - rootRect.top + offsetY;
    tooltip.style.transform = `translate(${x}px, ${y}px)`;
  };

  const handlePointerEnter = (event: PointerEvent, node: ForceNode) => {
  ...
  function syncThumbnailPatterns(nodes: ForceNode[]) {
    ...
    defs
      .selectAll<SVGPatternElement, ForceNode>("pattern.node-thumb")
      ...
      .join(
        (enter) => {
          const pattern = enter
            ...
            .attr("id", (d) => `thumb-${safePatternId(d.id)}`)
            .attr("width", 1)
            .attr("height", 1)
  ...
  const linkLayer = zoomLayer
    .append("g")
    .attr("stroke", typeof linkStroke === "string" ? linkStroke : null)
  ...
  const nodeLayer = zoomLayer
    .append("g")
    .attr("fill", typeof nodeFill === "string" ? nodeFill : null)
    .attr("stroke", typeof nodeStroke === "string" ? nodeStroke : null)
  ...
  if (typeof nodeStroke === "function") {
    nodeSelection.attr("stroke", (_d, i) => nodeStrokeValues![i]);
  }

  if (typeof nodeStrokeWidth === "function") {
    nodeSelection.attr("stroke-width", (_d, i) => nodeStrokeWidthValues![i]);
  }
  ...
  const bindNodeInteractions = (
    selection: d3.Selection<SVGRectElement, ForceNode, SVGGElement, unknown>,
  ) => {
    selection
      .on("pointerenter.tooltip", (event, node) =>
        handlePointerEnter(event as PointerEvent, node),
      )
      .on("pointermove.tooltip", (event) =>
        handlePointerMove(event as PointerEvent),
      )
      .on("pointerleave.tooltip", () => handlePointerLeave());
  ...
  let currentTransform = d3.zoomIdentity.scale(initialZoomScale);
  ...
  const zoomBehavior = d3
    ...
    .on("start", () => svg.style("cursor", "default"))
  ...
  svg
    ...
    .call(zoomBehavior.transform as any, currentTransform);
  ...
  const applyTheme = (nextTheme: ForceGraphThemeOptions) => {
    currentTheme = {
      ...currentTheme,
      ...nextTheme,
    };
    ...
    if (currentTheme.nodeStroke && typeof nodeStroke !== "function") {
      nodeSelection.attr("stroke", currentTheme.nodeStroke);
    }

    if (currentTheme.nodeStrokeOpacity !== undefined) {
      nodeSelection.attr("stroke-opacity", currentTheme.nodeStrokeOpacity);
    }

    if (
      currentTheme.nodeStrokeWidth !== undefined &&
      typeof nodeStrokeWidth !== "function"
    ) {
      nodeSelection.attr("stroke-width", currentTheme.nodeStrokeWidth);
    }

    if (currentTheme.linkStrokeOpacity !== undefined) {
      linkSelection.attr("stroke-opacity", currentTheme.linkStrokeOpacity);
    }
    ...
    if (typeof linkStroke === "function") {
      ...
      linkSelection.attr("stroke", currentTheme.linkStroke);
  ...
  if (typeof linkStrokeWidth === "function") {
    linkSelection.attr("stroke-width", getLinkStrokeWidth);
  }
  ...
  function getPosition(value: string | ForceNode, axis: "x" | "y"): number {
  ...
  function ticked() {
    linkSelection
      .attr("x1", (d) => getPosition(d.source, "x"))
      .attr("y1", (d) => getPosition(d.source, "y"))
      .attr("x2", (d) => getPosition(d.target, "x"))
      .attr("y2", (d) => getPosition(d.target, "y"));
  ...
  const updateData = (nextData: GraphData) => {
    ...
    const previousNodeIds = new Set(forceNodes.map((node) => node.id));
    const previousLinkKeys = new Set(
      forceLinks.map((link) => getLinkKey(link)),
    );
    const existingNodeById = new Map(
      forceNodes.map((node) => [node.id, node] as const),
    );
    const existingLinkByKey = new Map(
      forceLinks.map((link) => [getLinkKey(link), link] as const),
    );
    ...
    const nextForceNodes: ForceNode[] = currentNodes.map((node, index) => {
    ...
    const nextLinkKeys = currentLinks.map(
      (_link, index) => `${linkSources[index]}->${linkTargets[index]}`,
    );
    ...
    const nextForceLinks: ForceLink[] = currentLinks.map((link, index) => {
    ...
    if (typeof linkStrokeDasharray === "function") {
      linkSelection.attr("stroke-dasharray", (d, i) =>
        (linkStrokeDasharray as (link: GraphLink, index: number) => string)(
          d,
          i,
        ),
      );
    ...
    const joinedNodes = nodeLayer
      ...
      .join(
        (enter) => {
          ...
          if (typeof nodeFill === "function") {
            enterSelection.attr("fill", getNodeFill);
          } else if (typeof nodeFill === "string") {
            enterSelection.attr("fill", nodeFill);
          }
          if (typeof nodeStroke === "function") {
            enterSelection.attr("stroke", (_d, i) => nodeStrokeValues![i]);
          }
          if (typeof nodeStrokeWidth === "function") {
            enterSelection.attr(
              "stroke-width",
          ...
          return bindNodeInteractions(
            enterSelection.call(drag(simulation) as any),
          );
        },
    ...
    if (nodesChanged || linksChanged || nodePropsChanged) {
      ...
      simulation.alpha(Math.max(simulation.alpha(), targetAlpha)).restart();
    }

    applyTheme({});
  };

  function drag(sim: d3.Simulation<ForceNode, ForceLink>) {
    function stopEventPropagation(
      event: d3.D3DragEvent<SVGRectElement, ForceNode, ForceNode>,
    ) {
      (
  ...
  return {
    ...
    updateNodeFill: (fn: (node: GraphNode, index: number) => string) => {
      nodeFill = fn;
      nodeFillValues = currentNodes.map((node, i) => fn(node, i));
      nodeSelection.attr("fill", (d, i) => {
        if (d.thumbnailUrl) {
          return `url(#thumb-${safePatternId(d.id)})`;
        }
    ...
    updateNodeStroke: (fn: (node: GraphNode, index: number) => string) => {
      nodeStroke = fn;
      nodeStrokeValues = currentNodes.map((node, i) => fn(node, i));
      nodeSelection.attr("stroke", (_d, i) => nodeStrokeValues![i]);
    },
    updateNodeStrokeWidth: (fn: (node: GraphNode, index: number) => number) => {
      nodeStrokeWidth = fn;
      nodeStrokeWidthValues = currentNodes.map((node, i) => fn(node, i));
      nodeSelection.attr("stroke-width", (_d, i) => nodeStrokeWidthValues![i]);
    },
    updateLinkStroke: (fn: (link: GraphLink, index: number) => string) => {
      linkStroke = fn;
      linkStrokeColors = d3.map(currentLinks, fn);
      linkSelection.attr("stroke", (_d, i) => linkStrokeColors![i]);
    },
    resetView: () => {
      const baseTransform = d3.zoomIdentity.scale(initialZoomScale);
      currentTransform = baseTransform;
      svg.call(zoomBehavior.transform as any, baseTransform);
    },
    refreshLinkStyles: () => {
      if (typeof linkStrokeDasharray === "function") {
        linkSelection.attr("stroke-dasharray", (d, i) =>
      ...
      if (typeof linkStrength === "function") {
        linkForce.strength(
          linkStrength as (link: GraphLink, index: number) => number,
        );
    ...
    destroy: () => {
      svgElement.removeEventListener("wheel", handleWheelPan);
      hideTooltip();
      tooltip.remove();
    },
  };
