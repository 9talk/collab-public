...
function legacyBreakToSentinel(markdown: string): string {
  return markdown
    .split("\n")
    .map((line) =>
      EMPTY_PARA_LEGACY_BR.test(line) ? EMPTY_PARA_SENTINEL : line,
    )
    .join("\n");
}
...
const WIKI_IMAGE_SAVE_RE = /!\[([^\]]*)\]\(wikiimage:((?:\([^)]*\)|[^)])+)\)/g;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function repositionCursorAfterDivider(
  editor: any,
  dividerBlockId: string,
): void {
  try {
  ...
  return (
    <ImageResolverContext.Provider value={imageContext}>
      <div className="item-body">
        ...
        />
        {!editingDisabled && <WikiLinkAutocomplete editor={editor} />}
      </div>
