...
import type { ViewerItem, Quote, Concept, ItemSource } from "./types";
...
export function parseFileToViewerItem(
  ...
  const quotes: Quote[] | undefined = attributes.quotes?.map((q) =>
    typeof q === "string" ? { text: q } : q,
  );

  const concepts: Concept[] | undefined = attributes.concepts;
...
const STRIPPED_FIELDS = new Set(["createdAt", "modifiedAt", "author"]);

export function serializeViewerItem(item: ViewerItem, body: string): string {
  const attrs: Record<string, unknown> = {};
  const hadExplicitType = item.frontmatter?.type != null;
  if (hadExplicitType && item.type !== "skill") attrs.type = item.type;
  ...
  if (item.relatedConcepts?.length) attrs.concepts = item.relatedConcepts;
