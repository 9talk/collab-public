...
- `devtool_start`：启动项目服务；启动脚本必须为后台型（启动服务进程后立即退出，通过 stdout 上报 `COLLAB_PID:` 等约定标记），否则判定失败；启动成功后，在回复中向用户提示浏览器访问地址（如 `http://localhost:[port]`）；服务运行超过 24 小时会被自动关闭（防止遗忘的服务长期占用内存）
