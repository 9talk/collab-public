...
describe("challenge signature", () => {
  test("verify accepts genuine signature, rejects wrong key and tamper", () => {
    ...
    const parsed = decodeCredential(file);
    if (!parsed.ok) throw new Error("unreachable");
    const { clientPrivateKey, clientPublicKey, hostPublicKey } =
      parsed.credential;

    const sig = signChallenge(
      "collab-client-auth:v1",
      "nonce-abc",
      clientPrivateKey,
    );
    expect(
    ...
    // 域串不符
    expect(
      verifyChallenge("collab-host-auth:v1", "nonce-abc", clientPublicKey, sig),
    ).toBe(false);
    // nonce 篡改
    expect(
      verifyChallenge(
    ...
    expect(
      verifyChallenge("collab-client-auth:v1", "nonce-abc", hostPublicKey, sig),
    ).toBe(false);
  });
});
