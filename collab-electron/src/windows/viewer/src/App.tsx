...
export default function App() {
    ...
    return window.api.onWikilinksUpdated((updatedPaths) => {
      ...
      if (current && updatedPaths.includes(current)) {
        Promise.all([
          ...
          .catch((err) => {
            console.error("Failed to reload after wikilinks update:", err);
          });
      }
    });
  }, []);
  ...
  useEffect(() => {
    if (!selectedPath || isImageFile(selectedPath) || isPdfFile(selectedPath))
      return;
      ...
      Promise.all([
        ...
        .then(([content, stats]) => {
          ...
          if (
            ...
            content === lastWrittenContentRef.current
          ) {
            return;
          }
          setFileContent(content);
  ...
  useEffect(() => {
    const onFocus = () => {
      const currentPath = selectedPathRef.current;
      if (!currentPath || isImageFile(currentPath) || isPdfFile(currentPath))
        return;
  ...
  useEffect(() => {
    ...
    Promise.all([window.api.readFile(path), window.api.getFileStats(path)])
      .then(([content, stats]) => {
        if (
          (ENABLE_STALE_LOAD_GUARD &&
            latestLoadTokenRef.current !== loadToken) ||
          (ENABLE_STALE_LOAD_GUARD && selectedPathRef.current !== path)
        ) {
          return;
        }
        setFileContent(content);
  ...
  const editingDisabled =
    !!selectedPath && selectedPath !== loadedPath && !isRenamingRef.current;
  const displayedIsMarkdown = displayedPath
    ? isMarkdownFile(displayedPath)
    : false;
  const displayedIsImage = displayedPath ? isImageFile(displayedPath) : false;
  const displayedIsPdf = displayedPath ? isPdfFile(displayedPath) : false;
  const showFileLoading = !!selectedPath && !displayedPath && !fileError;
  const hasMarkdownFile =
    !!viewerItem && !fileError && !!displayedPath && displayedIsMarkdown;
  const hasImageFile = !!displayedPath && !fileError && displayedIsImage;
  const hasPdfFile = !!displayedPath && !fileError && displayedIsPdf;
  const hasCodeFile =
    !!displayedPath &&
    !fileError &&
    !displayedIsMarkdown &&
  ...
  const headerPath = showFileLoading ? selectedPath : displayedPath;
    ...
    <div className={`app${navVisible ? " nav-visible" : ""}`}>
      ...
      <main className="main-content scrollbar-hover">
        ...
        {hasFolder && (
          <>
            {!isTileMode && (
              <CloseOverlay onClick={handleCloseFolder} theme={theme} />
            )}
            <FolderTableView
          ...
          <div className={`item-card${isTileMode ? " canvas-tile-embed" : ""}`}>
            <div className="item-metadata">
              <div className="metadata-title-row">
                ...
                <span className="metadata-value item-metadata-title">
                  {viewerItem.title}
                </span>
              </div>
              ...
              {viewerItem.isEditable && (
                <div className="metadata-item">
                  ...
                  <div className="metadata-value">
                    {new Date(viewerItem.modifiedAt).toLocaleDateString()}
                  </div>
                </div>
              )}
            </div>
