...
import { saveExpandedWorkspaces } from "@collab/components/TreeView/useFileTree";
import {
  displayBasename,
  isSubpath,
  parentPath,
...
function ImportWebArticleModal({
  ...
  const handleImport = async () => {
    ...
    setError(null);
    try {
      const result = await window.api.importWebArticle(url.trim(), folderPath);
...
export default function App() {
  ...
  const [pendingExpandAll, setPendingExpandAll] = useState<Set<string>>(
    new Set(),
  );
  ...
  const workspaceRefsMap = useRef(
    new Map<string, React.RefObject<WorkspaceFileTreeHandle | null>>(),
  );

  const getWorkspaceRef = useCallback((wsPath: string) => {
    let ref = workspaceRefsMap.current.get(wsPath);
    if (!ref) {
      ref = React.createRef<WorkspaceFileTreeHandle>();
      workspaceRefsMap.current.set(wsPath, ref);
    }
    return ref;
  }, []);
  ...
  const getAllFlatItems = useCallback(() => {
    ...
    for (const ws of sortedWorkspaces) {
      ...
      const ref = workspaceRefsMap.current.get(ws.path);
      if (ref?.current) {
  ...
  useEffect(() => {
    ...
    const onLeave = (e: MouseEvent) => {
      ...
      if (!leaving) return;
      const entering = (e.relatedTarget as Element)?.closest?.(
        "[data-tooltip]",
      );
  ...
  useEffect(() => {
    window.api.getPref(TREE_SORT_MODE_STORAGE_KEY).then((v) => {
    ...
    window.api
      ...
      .then((stored) => {
        ...
        if (Array.isArray(stored) && stored.length > 0) {
          ...
          // Default: expand all
          setExpandedWorkspaces(new Set(workspacePaths.map((p) => p)));
        }
      })
      .catch(() => {});
  ...
  useEffect(() => {
    const changed = workspacesKey !== prevWorkspacesKeyRef.current;
    ...
    if (changed) {
      ...
      setExpandedWorkspaces((prev) => {
        ...
        if (valid.size === 0 && !suppressWorkspaceExpandRef.current)
          return roots;
        return valid;
      });
  ...
  useEffect(() => {
    const cleanupAdd = window.api.onWorkspaceAdded((path: string) => {
    ...
    const cleanupRemove = window.api.onWorkspaceRemoved((path: string) => {
      setWorkspacePaths((prev) => prev.filter((p) => p !== path));
      setSelectedPath((current) =>
        current?.startsWith(path + "/") ? null : current,
      );
    });
    return () => {
      cleanupAdd();
  ...
  async function createFolderInFolder(parentFolder: string) {
    ...
    const entries = await window.api.readDir(parentFolder);
    const existingNames = new Set(entries.map((e) => e.name.toLowerCase()));
  ...
  const stableDragStart = useCallback(
    (e: React.DragEvent, path: string) =>
      dragDrop.handleDragStart(e, path, multiSelectRef.current.selected),
  ...
  const cycleSortMode = useCallback(() => {
    setTreeSortMode((currentMode) => {
      ...
      window.api.setPref(TREE_SORT_MODE_STORAGE_KEY, newMode);
      return newMode;
    });
  }, []);
  ...
  const handleContextMenu = useCallback(
    async (_e: React.MouseEvent, item: FlatItem | null) => {
      ...
      const action = await window.api.showContextMenu(menuItems);
      if (!action) return;
      ...
      switch (action) {
        ...
        case "reveal-in-finder":
          if (item) window.api.revealInFinder(item.path);
          break;
        case "terminal":
          if (item)
            window.api.openInTerminal(
              item.kind === "folder" || item.kind === "workspace"
                ? item.path
                : parentPath(item.path),
            );
          break;
  ...
  const toggleWorkspace = useCallback((path: string, recursive: boolean) => {
    setExpandedWorkspaces((prev) => {
  ...
  const navigateItems = useCallback(
    (direction: "up" | "down", shiftKey: boolean) => {
      ...
      for (const ws of sortedWorkspaces) {
        ...
        const ref = workspaceRefsMap.current.get(ws.path);
        if (ref?.current) {
          allNavigable.push(...ref.current.navigableItems);
        }
      }
      if (allNavigable.length === 0) return;

      const effectivePath = multiSelect.cursor ?? selectedPath;
      let currentIndex = allNavigable.findIndex(
      ...
      if (direction === "down") {
        nextIndex =
          currentIndex < 0
            ? 0
            : Math.min(currentIndex + 1, allNavigable.length - 1);
      } else {
              ...
              >
                {sortedWorkspaces.map((ws, idx) => (
                  <WorkspaceTree
                    ...
                    aliasingPath={inlineAlias.aliasingPath}
                    aliasValue={inlineAlias.aliasValue}
                    aliasInputRef={inlineAlias.inputRef}
