...
export function createEdgeIndicators({
  ...
  function panToTile(tile, { targetZoom } = {}) {
    ...
    const targetX = vw / 2 - (tile.x + tile.width / 2) * endZoom;
    const targetY = vh / 2 - (tile.y + tile.height / 2) * endZoom;
    ...
    function step(now) {
      ...
      if (targetZoom != null) {
        viewportState.zoom = startZoom + (endZoom - startZoom) * e;
      }
      onViewportUpdate();
    ...
    for (const tile of tiles) {
      ...
      if (dot) {
        ...
        dot.addEventListener("mouseenter", () => {
          ...
          if (ctx) {
            showTooltip(dot, ctx.tile, ctx.dotX, ctx.dotY, ctx.vw, ctx.vh);
          }
        });
