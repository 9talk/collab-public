...
function getPanelConstraints(side) {
  ...
  const min = parseInt(s.getPropertyValue(`--panel-${side}-min`).trim(), 10);
  const max = parseInt(s.getPropertyValue(`--panel-${side}-max`).trim(), 10);
  return { min, max };
}
...
export function createPanel(side, config) {
  ...
  function updateTogglePosition() {
    ...
    if (direction === 1) {
      ...
      if (mode !== "closed") {
        const rect = panel.getBoundingClientRect();
        toggle.style.right = `${panelsRect.right - rect.left + 8}px`;
      } else {
  ...
  function applyVisibility() {
    ...
    if (visible) {
      ...
      const px = stored != null && stored > 1 ? stored : defaultWidth;
  ...
  function setupResize(onResize = () => {}) {
    ...
    resizeHandle.addEventListener("mousedown", (e) => {
      ...
      function onMouseUp() {
        ...
        savePref(`panel-width-${side}`, panel.getBoundingClientRect().width);
      }
