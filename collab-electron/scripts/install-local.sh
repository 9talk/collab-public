...
# Build, install and verify a single flavor.
install_flavor() {
  local FLAVOR="$1"
  local APP_NAME DIST_DIR PACKAGE_CMD DEST_APP SOURCE_APP
  case "$FLAVOR" in
    full)
  ...
  # Step 5: Clean up this flavor's build artifacts (unless --keep)
  if [ "$KEEP" = false ]; then
    echo "Cleaning up $DIST_DIR/..."
    rm -rf "$PROJECT_DIR/$DIST_DIR"
  fi

  echo "Done. $APP_NAME installed at $DEST_APP"
}

install_flavor full
install_flavor remote
...
echo ""
echo "Both apps installed: /Applications/Collaborator.app + /Applications/Collaborator Remote.app"
