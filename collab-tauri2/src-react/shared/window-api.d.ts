// Tauri adapter type definitions — mirrors the Electron CollabApi interface.
// These types are re-exported for use by components and the adapter.
...
export type PtyDataCb = (
  payload: { sessionId: string; data: Uint8Array },
) => void;
export type PtyExitCb = (
  payload: { sessionId: string; exitCode: number },
) => void;
export type CdToCb = (path: string) => void;
export type RunInTerminalCb = (command: string) => void;
...
export interface AgentFileTouchedEvent {
  kind: "file-touched";
  sessionId: string;
  filePath: string;
...
export type { AppConfig, FolderTableData, TreeNode } from "../shared/types";
export type { ReplayMessage } from "../shared/replay-types";
